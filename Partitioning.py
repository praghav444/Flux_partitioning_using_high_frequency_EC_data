...
"""
Author: Pushpendra Raghav & Einara Zahn <ppushpendra@crimson.ua.edu> | <einaraz@princeton.edu>
...
class Partitioning(object):
    ...
    * Note that CEC and MREA only need time series of w_p, c_p, q_p
    ...
    """
    def __init__(self, height_file, df):  # Modified by Raghav (instead of single height, provide a csv file with height information)
        datetime = df.index[0]
        date = datetime.date()
        heights = _lookup(height_file, 0, 1, 2)   # height file with column0: Date, column1: canopy height, and column3: measurement ht
        hi = heights(date)[0]
        zi = heights(date)[1]
    ...
    def WaterUseEfficiency(self, ppath='C4'):
        """
        ...
        if ppath == "C3":
           ...
           # self.wue['opt'] = np.nan  # Model is not suitable for C4 and CAM plants
            #-------------# Edited (Raghav)-----(Run opt for C4 plants also)
            #-------------
            m = - ( varc*cov_wq - corr_qc*sigmaq*sigmac*cov_wc )/( varq*cov_wc - corr_qc*sigmaq*sigmac*cov_wq )
            vpdm = vapor_press_deficit_mass(ambient_h2o, leaf_T, Constants.Rvapor)
            if vpdm < 0 or m < 0:
